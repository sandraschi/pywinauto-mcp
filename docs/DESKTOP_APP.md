...
|----------------|------------|
| `pywinauto-mcp-operator.exe` | **The product.** Tauri + WebView + embedded React UI + embedded Python backend |

No second exe in the install folder. The PyInstaller backend is bundled as an app **resource**, copied on first launch to `%LOCALAPPDATA%\com.sandraschi.pywinauto-mcp\cache\`, spawned as a child process, and killed when you quit.
...
```
pywinauto-mcp-operator.exe          (single user-facing binary)
  ├── web_sota/dist                 (embedded React UI)
  └── resources/pywinauto-mcp-backend.exe   (embedded, not a sibling shortcut)
...
**Normal use:** run `pywinauto-mcp-operator.exe` only.

**Debugging without Tauri:** build the backend with `npm run sidecar:build`, then run `web_sota\src-tauri\resources\pywinauto-mcp-backend.exe` (or the dev copy under `binaries/`).
...
cd web_sota
npm run sidecar:build
.\src-tauri\resources\pywinauto-mcp-backend.exe
```
