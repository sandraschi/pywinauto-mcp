...
**9. `src/pywinauto_mcp/main.bak` in source tree. Delete.**

**10. `coverage.xml`, `htmlcov/`, `ruff_stats*.txt`, `paint_elements.json`, `pywinauto-mcp.log` committed to repo root. Gitignore and clean.**
...
**14. `trajectory.py` ENV var is `PYWINAUTO_MCP_TRAJECTORY_LOG` but `cua_env.py` transition shim is not used — it should use `cua_truthy("CUA_MCP_TRAJECTORY_LOG", "PYWINAUTO_MCP_TRAJECTORY_LOG")`.**
...
- `pywinauto-mcp profile add --app vroidstudio --stable-region 280,120,1640,980` CLI subcommand
...
Remaining steps:
1. Module rename `pywinauto_mcp` → `cua_mcp` (breaking — requires vroidstudio-mcp update)
2. All env vars use `cua_truthy`/`cua_getenv` (trajectory.py still uses raw `os.getenv`)
3. PyPI: publish `cua-mcp`, keep `pywinauto-mcp` as deprecated wrapper
