...
Formerly `pywinauto-mcp`. Nobody knows what pywinauto is; this server is a **computer use agent** for Windows apps that lack an API.
...
| pywinauto-mcp (current) | Describes implementation detail | Sounds like a Python wrapper, not an agent |

**Decision:** Rename repo and display name to **cua-mcp**.

**Transition plan:**
1. v0.4.x: display name + docs say CUA-MCP; PyPI package stays `pywinauto-mcp`
2. v0.5.x: publish `cua-mcp` on PyPI; `pywinauto-mcp` becomes deprecated alias
3. v1.0.x: env vars `CUA_MCP_*` (fallback to `PYWINAUTO_MCP_*`); module `pywinauto_mcp` → `cua_mcp`
...
| 8 | `automation_mission` record/replay + trajectory | todo |
| 9 | Vision polish (multi-match, evidence bundles, region masks) | todo |
...
1. PyPI: publish `cua-mcp`, keep `pywinauto-mcp` as deprecated alias
2. Env vars: `CUA_MCP_*` with fallback to `PYWINAUTO_MCP_*`
3. Module: `pywinauto_mcp` → `cua_mcp` (breaking; major version bump)
...
| Repo | Port | Role |
|------|------|------|
| pywinauto-mcp / cua-mcp | 10789 | Windows GUI agent (this repo) |
