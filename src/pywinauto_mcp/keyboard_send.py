...
from pywinauto_mcp.cua_env import keyboard_backend


def send_hotkey(keys: Sequence[str], *, hwnd: int | None = None, pause: float = 0.0) -> dict:
    backend = keyboard_backend()
    if backend == "win32":
        from pywinauto_mcp.win32_keyboard import send_hotkey as win32_hotkey
...
def send_press(key: str, *, hwnd: int | None = None, presses: int = 1, pause: float = 0.0) -> dict:
    backend = keyboard_backend()
    if backend == "win32":
        from pywinauto_mcp.win32_keyboard import send_press as win32_press
