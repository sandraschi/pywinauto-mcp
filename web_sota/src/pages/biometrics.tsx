...
import { Activity, ListOrdered, ScanFace, ShieldCheck, Trash2 } from "lucide-react";
...
export function Biometrics() {
    ...
    const [faceBusy, setFaceBusy] = useState<null | "list" | "capture" | "delete">(null);
    ...
    const hitlActive =
        safety?.hitl?.safe_window_until != null && safety.hitl.safe_window_until > Date.now() / 1000;
    ...
    const runFace = async (op: "list" | "capture" | "delete") => {
        ...
        try {
            ...
            const inner = res.raw.result as { status?: string; error?: string } | undefined;
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-2">
                <h1 className="text-3xl font-bold tracking-tight text-white">Biometrics</h1>
                <p className="text-slate-400">
                    Camera selection matches the REST/OpenCV list for <code className="text-slate-500">camera_index</code>
                    . The live preview uses the browser&apos;s <strong className="text-slate-300">video device order</strong>
                    — if preview and OpenCV disagree, fix the default camera in Windows Settings.
                </p>
            </div>
            ...
            <Card className="border-slate-800 bg-slate-950/50">
                ...
                <CardContent className="pt-6">
                    ...
                    <p className="text-xs text-slate-500 mt-3">
                        OpenCV index <code className="text-slate-400">{cameraIndex}</code> for{" "}
                        <code className="text-slate-400">automation_face</code> — see{" "}
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                <Card className="border-slate-800 bg-slate-950/50 overflow-hidden">
                    <CardHeader className="border-b border-slate-800 flex flex-row items-center justify-between py-3">
                        ...
                        <Badge variant="secondary" className="bg-emerald-950/40 text-emerald-400 border-emerald-800/50">
                            Browser
                        </Badge>
                    </CardHeader>
                ...
                <Card className="border-slate-800 bg-slate-950/50">
                    ...
                    <CardContent className="space-y-3 pt-4 text-sm">
                        ...
                        {!safety && !safetyErr && <p className="text-slate-500 text-xs">Loading…</p>}
                        {safety && (
                            <>
                                <div className="flex justify-between gap-2 border border-slate-800 rounded-lg p-3 bg-slate-900/40">
                                    ...
                                    <span
                                        className={
                                            safety.face_tool_opt_in ? "text-emerald-400 font-medium" : "text-slate-500"
                                        }
                                    >
                                ...
                                <div className="flex justify-between gap-2 border border-slate-800 rounded-lg p-3 bg-slate-900/40">
                                    ...
                                    <span
                                        className={
                                            safety.snapshot?.kill_switch ? "text-red-400 font-medium" : "text-slate-300"
                                        }
                                    >
                                ...
                                <div className="flex justify-between gap-2 border border-slate-800 rounded-lg p-3 bg-slate-900/40">
                                    ...
                                    <span className={safety.snapshot?.dry_run ? "text-amber-400" : "text-slate-300"}>
                                ...
                                <p className="text-xs text-slate-500">
                                    Mutations (60s): {safety.snapshot?.actions_last_60s ?? "—"} / cap{" "}
                                    {safety.snapshot?.max_actions_per_minute ?? "—"}
                                </p>
                            </>
                        )}
                    </CardContent>
                ...
                <CardContent className="space-y-4 pt-4">
                    {!faceRegistered && (
                        <p className="text-sm text-amber-300/90">
                            <code className="text-amber-200/90">automation_face</code> is not registered. Set{" "}
                            <code className="text-amber-200/90">PYWINAUTO_MCP_ENABLE_FACE=1</code>, install the{" "}
                            <code className="text-amber-200/90">face</code> extra, restart the server, then refresh this
                            page.
                        </p>
                    )}
                    {faceRegistered && (
                        <p className="text-xs text-slate-500">
                            ...
                            <code className="text-slate-400">camera_index</code> for capture. See{" "}
                            <code className="text-slate-400">docs/SAFETY.md</code> §5.
                        </p>
                    )}
                    ...
                    <div className="flex flex-wrap items-end gap-3 max-w-md">
                        <div className="grid gap-1.5 flex-1 min-w-[12rem]">
                            <Label className="text-slate-400 text-xs">Delete profile name</Label>
                            <Input
                        ...
                        <Button
                            ...
                            disabled={!faceRegistered || faceBusy !== null || !deleteName.trim()}
