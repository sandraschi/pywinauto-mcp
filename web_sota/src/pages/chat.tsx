...
export function Chat() {
    ...
    useEffect(() => {
        ...
        fetch(apiPath("/api/v1/llm/config"))
            ...
            .catch(() => setError("Could not load LLM config (is the backend running?)"));
    ...
    const buildSystemPrompt = () => {
        ...
        if (includeRepo && repoMarkdown.trim()) {
            parts.push("--- Repository knowledge (pywinauto-mcp) ---\n" + repoMarkdown);
        }
        return parts.join("\n\n");
    };
    ...
    const send = async () => {
        ...
        try {
            ...
            const out = data as { content?: string | null; raw_error?: string | null };
            if (out.raw_error) {
    ...
    return (
        <div className="flex flex-col gap-4 pb-8">
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-white">Local LLM chat</h2>
                    <p className="text-slate-400 text-sm max-w-2xl">
                        ...
                        <code className="text-xs text-slate-500">1234</code>). Personas + repo knowledge answer questions
                        like &quot;can I click and drag?&quot; with project-accurate context.
                    </p>
                </div>
            ...
            <Card className="border-slate-800 bg-slate-950/50">
                <CardHeader className="pb-2">
                    <CardTitle className="text-white text-base">Connection & model</CardTitle>
                </CardHeader>
                <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    <div className="space-y-2">
                        <Label className="text-slate-400">API base (OpenAI-compatible /v1)</Label>
                        <input
                    ...
                    <div className="space-y-2 flex flex-col justify-end">
                        ...
                        <p className="text-xs text-slate-500">
                            Injects curated facts from <code className="text-slate-400">llm_repo_context.py</code> (tools,
                            safety, drag/click scope).
                        </p>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card className="flex-1 border-slate-800 bg-slate-950/50 flex flex-col min-h-[420px] overflow-hidden">
                <CardContent className="flex flex-col flex-1 p-0 min-h-0">
                        ...
                        <div className="space-y-4 pr-4">
                            {messages.length === 0 && (
                                <p className="text-slate-500 text-sm">
                                    Ask about this repo: e.g. whether you can click-and-drag, what requires approval, or
                                    which tool lists windows.
                                </p>
                            )}
                            {messages.map((m, i) => (
                    ...
                    <div className="p-4 border-t border-slate-800 bg-slate-900/30 space-y-3">
                        ...
                        <p className="text-xs text-slate-500">
                            Enter sends; Shift+Enter newline. Ensure Ollama or LM Studio is running and a model is loaded.
                        </p>
                    </div>
                </CardContent>
            </Card>
