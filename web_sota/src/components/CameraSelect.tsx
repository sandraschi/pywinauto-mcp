...
import { RefreshCw, Loader2, Video } from "lucide-react";
...
export function CameraSelect({
    ...
    return (
        <div className="space-y-2">
            ...
            {!loading && cameras.length === 0 && (
                <p className="text-xs text-slate-500">No cameras detected. Check drivers and privacy settings.</p>
            )}
            ...
            {multiple && (
                <p className="text-xs text-slate-500">Multiple cameras found — choose which device to use for capture.</p>
            )}
        </div>
    );
}
