...
Write-Host "=== pywinauto-mcp MCPB Package Builder ===" -ForegroundColor Cyan
...
$srcPackage = Join-Path $RepoRoot "src\pywinauto_mcp"
$dstPackage = Join-Path $RepoRoot "mcpb\src\pywinauto_mcp"
...
Write-Host "Synced src/pywinauto_mcp -> mcpb/src/pywinauto_mcp" -ForegroundColor Green
...
$packagePath = Join-Path $distDir "pywinauto-mcp.mcpb"
...
try {
    ...
    & $McpbExe pack "." (Join-Path $relOut "pywinauto-mcp.mcpb")
    if ($LASTEXITCODE -ne 0) {
